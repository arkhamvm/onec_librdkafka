...
* нет `MANIFEST.xml` и нет шага, который паковал бы компоненту в ZIP для макета
  внешней компоненты (`include/MANIFEST.xsd` — это просто схема из SDK 1С, она ни во
  что не собирается);
